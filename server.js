...
const EXCHANGES = {
    ...
    huobi: {
        ...
        getPrice: async (symbol) => {
            try {
                const response = await axios.get(`https://api.huobi.pro/market/detail/merged?symbol=${symbol.toLowerCase()}usdt`, {
                    timeout: 2000
                });
    ...
    gate: {
        ...
        getPrice: async (symbol) => {
            try {
                const response = await axios.get(`https://api.gateio.ws/api/v4/spot/tickers?currency_pair=${symbol}_USDT`, {
                    timeout: 2000
                });
    ...
    kucoin: {
        ...
        getPrice: async (symbol) => {
            try {
                const response = await axios.get(`https://api.kucoin.com/api/v1/market/orderbook/level1?symbol=${symbol}-USDT`, {
                    timeout: 2000
                });
